...
function reducer(state: CartState, action: CartAction): CartState {
  switch (action.type) {
    case "ADD": {
      ...
      if (existing) {
        return { ...state, items: state.items.map((i) => i.product.id === action.product.id ? { ...i, qty: i.qty + 1 } : i) };
      }
...
export function CartProvider({ children }: { children: ReactNode }) {
  ...
  return (
    <CartContext.Provider value={{ state, add, remove, setQty, clear, openCart, closeCart, totalItems }}>
      {children}
